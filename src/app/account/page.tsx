...
export default function AccountPage() {
	...
	const upgradeToPremium = async () => {
		const priceId = 'price_1O1HnwCFINHqqFHkTptwKvgt';
		const checkoutUrl = await getCheckoutUrl(app, priceId);
		router.push(checkoutUrl);
		console.log('Upgrade to Premium');
	};

	const downgradeToStandard = async () => {
		const priceId = 'price_1O1FlgCFINHqqFHk5vGD6RYx';
		const checkoutUrl = await getCheckoutUrl(app, priceId);
		router.push(checkoutUrl);
		console.log('Downgrade to Standard');
	};
	...
	const upgradeToPremiumButton = (
		<div className='flex justify-between'>
			...
			>
				<div className='flex gap-2 items-center align-middle justify-center'>Standar plan</div>
			</button>
			<button
				onClick={upgradeToPremium}
				className='bg-blue-600 p-4 px-6 text-lg rounded-lg hover:bg-blue-700 shadow-lg'
			>
				<div className='flex gap-2 items-center align-middle justify-center'>Primium plan</div>
			</button>
		</div>
	);
	...
	const managePortalButton = (
		<button
			...
			className='bg-blue-600 p-4 px-6 text-lg rounded-lg hover:bg-blue-700 shadow-lg'
		>
			<div className='flex gap-2 items-center align-middle justify-center'>Manage Subscription</div>
		</button>
	);
	...
	const signOutButton = (
		...
		>
			<div className='flex gap-2 items-center align-middle justify-center'>Sign Out</div>
		</button>
	);
